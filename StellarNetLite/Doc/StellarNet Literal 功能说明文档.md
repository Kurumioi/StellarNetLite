...
- 房间消息接入
- 轻状态缓存
- **事件系统 (GlobalTypeEvent / RoomEventSystem)**
- Replay 播放器
- View 输入桥接
...
## 15.3 当前事件系统解决的问题
它解决了：
- 繁琐的中间 DTO (Event 结构体) 转换
- View 忘记注销导致的内存泄漏 (通过 `UnRegisterWhenGameObjectDestroyed`)
- 在线房间与回放房间串线
- 全局事件和房间事件搅在一起
