...
### 10. 结构化日志
提供统一日志入口 `NetLogger`，统一输出模块名、RoomId、SessionId 等上下文，便于多人联机、回放、断线等场景快速排查。
...
**承载预估 (单服节点)：**
- **并发规模**：100 ~ 300 CCU（取决于同步频率，受限于 Unity 主线程与 JSON 序列化开销）。
- **房间规模**：4 ~ 16 人（高频空间/动画同步，如交友房间），或 50 人以内（低频状态同步，如棋牌/回合制）。

**适合：**
- 轻中量级多人合作游戏 / 房间制 PVE / 桌游棋牌 / 社交元宇宙
- 中小型商业项目
- 需要回放、断线重连、房间组件化管理的项目

**不适合：**
- 高竞技强对抗 PVP / 强预测回滚 / 严苛帧级命中公平判定
- 单服万人级别的 MMO（需剥离主线程并引入网关集群）
...
### 客户端主链路
收到服务端协议 -> `ClientApp.OnReceivePacket()` -> 按 `Scope` 路由 -> 对应 `ClientModule / ClientRoomComponent` 处理 -> 直接通过 `GlobalTypeNetEvent` 或 `Room.NetEventSystem` 抛出协议对象 -> View 层监听协议并刷新表现。
...
├── Editor            (协议扫描器、预制体扫描器、配置面板、脚手架)
└── GameDemo          (示例业务代码与 UI 路由)
```
...
2. **场景挂载**：在启动场景中挂载 `GameLauncher` 与 `StellarNetMirrorManager`。
3. **配置网络**：点击菜单 `StellarNet/Lite 网络配置 (NetConfig)`，配置 IP、端口、TickRate 等参数并保存。
4. **运行测试**：在 Editor 中测试 Host、Server Only 或 Client Only 模式。
...
在正式脱离 Demo 进行业务开发时，你需要通过代码直接驱动框架。以下是客户端最核心的网络流转 API 调用范例。
...
### 2. 状态轮询与界面跳转
客户端不应直接监听底层的登录成功协议，而是通过轮询 `NetClient.State` 或监听 `ClientUIRouter` 的事件来驱动 UI 跳转。
...
1. **新业务统一走强类型发送器**：绝对不要手写 `new Packet(...)`，统一使用 `NetClient.Send<T>()` 和 `Room.BroadcastMessage<T>()`。
