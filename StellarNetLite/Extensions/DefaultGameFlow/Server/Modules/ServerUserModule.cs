    ...
    public sealed class ServerUserModule
    {
        ...
        public void OnC2S_Login(Session session, C2S_Login msg)
        {
            ...
            if (oldSession != null)
            {
                ...
                {
                    bool hasRoom = TryGetRecoverableRoom(oldSession, out _, out _);
                    _app.SendMessageToSession(
                        oldSession,
                ...
                bool hasReconnectRoom = TryGetRecoverableRoom(oldSession, out _, out _);
                _app.SendMessageToSession(
                    oldSession,
        ...
        public void OnC2S_ConfirmReconnect(Session session, C2S_ConfirmReconnect msg)
        {
            ...
            if (!TryGetRecoverableRoom(session, out Room roomOut, out string reason))
            {
                _app.SendMessageToSession(session, new S2C_ReconnectResult { Success = false, Reason = reason });
                NetLogger.LogWarning("ServerUserModule", $"重连拒绝: {reason}", roomId, session.SessionId);
                return;
            }

            _app.SendMessageToSession(
                session,
                new S2C_ReconnectResult { Success = true, RoomId = roomOut.RoomId, ComponentIds = roomOut.ComponentIds });
            NetLogger.LogInfo(
                "ServerUserModule",
                $"重连确认完成: ComponentCount:{roomOut.ComponentIds.Length}",
                roomOut.RoomId,
                session.SessionId);
        }
        ...
        public void OnC2S_ReconnectReady(Session session, C2S_ReconnectReady msg)
        {
            ...
            {
                _app.SendMessageToSession(session, new S2C_RoomSetupResult { Success = false, RoomId = string.Empty, Reason = "当前没有可恢复房间" });
                NetLogger.LogWarning("ServerUserModule", "重连就绪跳过: 当前未绑定房间", sessionId: session.SessionId);
                return;
            }

            if (!TryGetRecoverableRoom(session, out Room room, out string reason))
            {
                _app.SendMessageToSession(session, new S2C_RoomSetupResult { Success = false, RoomId = roomId, Reason = reason });
                NetLogger.LogWarning("ServerUserModule", $"重连就绪跳过: {reason}", roomId, session.SessionId);
                return;
            }
        ...
        private bool TryGetRecoverableRoom(Session session, out Room room, out string reason)
        {
            room = null;
            reason = string.Empty;
