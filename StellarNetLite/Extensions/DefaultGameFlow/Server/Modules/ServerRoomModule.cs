    ...
    public sealed class ServerRoomModule
    {
        ...
        public void OnC2S_RoomSetupReady(Session session, C2S_RoomSetupReady msg)
        {
            ...
            if (string.IsNullOrEmpty(msg.RoomId))
            {
                ...
                _app.SendMessageToSession(session, new S2C_RoomSetupResult { Success = false, RoomId = string.Empty, Reason = "必须指定有效的 RoomId" });
            ...
            {
                session.ClearAuthorizedRoom();
                _app.SendMessageToSession(session, new S2C_RoomSetupResult { Success = false, RoomId = msg.RoomId, Reason = "房间授权已失效" });
            ...
            {
                session.ClearAuthorizedRoom();
                _app.SendMessageToSession(session, new S2C_RoomSetupResult { Success = false, RoomId = msg.RoomId, Reason = "已在房间中" });
            ...
            {
                session.ClearAuthorizedRoom();
                _app.SendMessageToSession(session, new S2C_RoomSetupResult { Success = false, RoomId = msg.RoomId, Reason = "房间不存在" });
