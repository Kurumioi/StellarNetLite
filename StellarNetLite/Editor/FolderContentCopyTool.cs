    ...
    public class FolderContentCopyTool : EditorWindow
    {
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                if (_optimizeForAI)
                {
                    using (new GUILayout.HorizontalScope())
                    {
                        GUILayout.Space(20);
                        _removeComments =
                            EditorGUILayout.ToggleLeft("移除注释 (//...)", _removeComments, GUILayout.Width(140));
                        _removeIndentation =
                            EditorGUILayout.ToggleLeft("移除缩进 (扁平化)", _removeIndentation, GUILayout.Width(140));
                    }
        ...
        private void CopySelectedFoldersToClipboard()
        {
            ...
            try
            {
                ...
                {
                    folderIndex++;
                    EditorUtility.DisplayProgressBar("处理中", $"扫描目录: {Path.GetFileName(folder)}",
                        (float)folderIndex / _selectedFolders.Count);
