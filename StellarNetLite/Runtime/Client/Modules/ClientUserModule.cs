    ...
    public sealed class ClientUserModule
    {
        ...
        public void OnS2C_LoginResult(S2C_LoginResult msg)
        {
            ...
            if (msg.Success)
            {
                ...
                Debug.Log($"[ClientUserModule] 登录成功, SessionId: {msg.SessionId}");

                if (msg.HasReconnectRoom)
                {
                    Debug.Log("[ClientUserModule] 发现可重连房间，等待玩家选择...");
                }
            }
            else
            {
                Debug.LogError($"[ClientUserModule] 登录失败: {msg.Reason}");
            }
        }
        ...
        public void OnS2C_ReconnectResult(S2C_ReconnectResult msg)
        {
            ...
            {
                Debug.Log($"[ClientUserModule] 重连结束: {msg.Reason}");
            }
        }
