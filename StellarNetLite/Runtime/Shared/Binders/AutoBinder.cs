...
namespace StellarNet.Lite.Shared.Binders
{
    public static class AutoBinder
    {
        ...
        static AutoBinder()
        {
            _createServerActionMethod = typeof(AutoBinder).GetMethod(nameof(CreateServerAction),
                BindingFlags.NonPublic | BindingFlags.Static);
            _createClientActionMethod = typeof(AutoBinder).GetMethod(nameof(CreateClientAction),
                BindingFlags.NonPublic | BindingFlags.Static);
        }

        // 核心修复：委托签名改为 Func<byte[], Type, object>，直接反序列化为新对象
        public static void BindServerModule(object moduleInstance, GlobalDispatcher dispatcher,
            Func<byte[], Type, object> deserializeFunc)
        {
            ...
            MethodInfo[] methods =
                moduleType.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            ...
            foreach (var method in methods)
            {
                ...
                var wrapper = (Action<Session, Packet>)genericHelper.Invoke(null,
                    new object[] { moduleInstance, method, deserializeFunc });
                dispatcher.Register(msgAttr.Id, wrapper);
            }
        }

        public static void BindServerComponent(RoomComponent componentInstance, RoomDispatcher dispatcher,
            Func<byte[], Type, object> deserializeFunc)
        {
            ...
            MethodInfo[] methods =
                compType.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            ...
            foreach (var method in methods)
            {
                ...
                MethodInfo genericHelper = _createServerActionMethod.MakeGenericMethod(msgType);
                var wrapper = (Action<Session, Packet>)genericHelper.Invoke(null,
                    new object[] { componentInstance, method, deserializeFunc });
                dispatcher.Register(msgAttr.Id, wrapper);
            }
        }

        public static void BindClientModule(object moduleInstance, ClientGlobalDispatcher dispatcher,
            Func<byte[], Type, object> deserializeFunc)
        {
            if (moduleInstance == null || dispatcher == null || deserializeFunc == null)
            {
            ...
            foreach (var method in methods)
            {
                ...
                var wrapper =
                    (Action<Packet>)genericHelper.Invoke(null,
                        new object[] { moduleInstance, method, deserializeFunc });
                dispatcher.Register(msgAttr.Id, wrapper);
            }
        }

        public static void BindClientComponent(ClientRoomComponent componentInstance, ClientRoomDispatcher dispatcher,
            Func<byte[], Type, object> deserializeFunc)
        {
            if (componentInstance == null || dispatcher == null || deserializeFunc == null)
            {
            ...
            foreach (var method in methods)
            {
                ...
                MethodInfo genericHelper = _createClientActionMethod.MakeGenericMethod(msgType);
                var wrapper = (Action<Packet>)genericHelper.Invoke(null,
                    new object[] { componentInstance, method, deserializeFunc });
                dispatcher.Register(msgAttr.Id, wrapper);
            }
        }

        private static Action<Session, Packet> CreateServerAction<TMsg>(object instance, MethodInfo method,
            Func<byte[], Type, object> deserializeFunc)
            where TMsg : class
        {
        ...
        private static Action<Packet> CreateClientAction<TMsg>(object instance, MethodInfo method,
            Func<byte[], Type, object> deserializeFunc)
            where TMsg : class
        {
